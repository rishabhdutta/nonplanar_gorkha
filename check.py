...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 31 18:50:10 2019
...
mat_samplestage = sio.loadmat('samples1/stage2/resampstage.mat')
...
covsmpl = mat_samplestage['covsmpl']
resmpl = mat_samplestage['resmpl']

indarray = 2#np.int(os.environ['arrayindex'])
numind = 4 
index = np.arange((indarray-1)*numind, indarray*numind)

NT1 = namedtuple('NT1', 'N Neff target LB UB')
opt = NT1(index, Nchains, postGorkha, LB, UB)

NT2 = namedtuple('NT2', 'allsamples postval beta stage covsmpl resmpl')
samples = NT2(samplestage, postval, beta[-1], stage[-1], covsmpl, resmpl)

current = SMCclass(opt, samples, NT1, NT2)
samples = current.MCMC_samples_part() 

varname = 'samples1/stage2/samples/sample2stage' + np.str(indarray)+ '.mat'    
